...
class Client
{
    ...
    public function prepareRequest($path, $method = 'GET', $data = '')
    {
        ...
        $this->setCurlOption(CURLOPT_FAILONERROR, 0);
    ...
    protected function runRequest($path, $method = 'GET', $data = '')
    {
        ...
        if($this->responseCode==403){
          curl_close($curl);
          // var_dump($response, $this->responseCode, $contentType);exit;
          throw new \Exception('Error 403 - Acesso Negado!',$this->responseCode);
        }
        ...
        if($response){
          $pos = strpos($response, '<errors type="array">');
          if($pos === false){
          }else{
            curl_close($curl);
            throw new \Exception($response);
